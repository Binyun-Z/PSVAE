...
def CreateDataSet(csv_list,args,alphabet):
    dflist = []
...
def main(net, train_dataloader, val_dataloader, loss, trainer, num_epochs, start_epoch, devices = utils.try_all_gpus()):
    ...
    #默认使用logger的级别
    fileHandler = logging.FileHandler(filename = "./ESM_finetuning/Finetuning_ESM_log/acc2.log")
    #格式
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if not os.path.isdir('./ESM_finetuning/checkpoint2'):
            os.mkdir('./ESM_finetuning/checkpoint2')
        ...
        if test_acc >= best_acc:
            ...
            torch.save(checkpoint, './ESM_finetuning/checkpoint2/bestmodel.pkl')
...
if __name__ == '__main__':
    ...
    train_dataloader,val_dataloader = CreateDataSet(csvlist,args=args,alphabet=alphabet)
    ...
    if RESUME:
        path_checkpoint = "./ESM_finetuning/checkpoint2/bestmodel.pkl"  # 断点路径
